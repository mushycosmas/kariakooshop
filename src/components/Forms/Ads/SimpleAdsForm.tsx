...
const SimpleAdsForm = () => {
  ...
  const updateTier = (
  index: number,
  field: 'min_qty' | 'max_qty' | 'whole_seller_price',
  value: string
) => {
  setForm((prev) => {
    ...
    if (field === 'whole_seller_price') {
      updated[index] = {
        ...tier,
        whole_seller_price: value,
      };
    } else if (field === 'min_qty') {
      updated[index] = {
        ...tier,
        min_qty: Number(value) || 0,
      };
    } else if (field === 'max_qty') {
      updated[index] = {
        ...tier,
        max_qty: Number(value) || 0,
      };
    }
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (
      ...
      setMessage('Please fill all required fields (including location)');
      return;
    }
        ...
        <Card.Body>
          ...
          <Row className="mb-3">
            ...
            <Col md={3}>
              ...
              <Form.Control type="number" name="min_order_qty" value={form.min_order_qty} onChange={handleChange} />
            </Col>
          </Row>
        ...
        <Card.Body>
          ...
          {form.wholesale_tiers.map((tier, i) => (
            <Row key={i} className="mb-2">
              <Col md={3}>
                <Form.Control type="number" value={tier.min_qty} onChange={(e) => updateTier(i, 'min_qty', e.target.value)} />
              </Col>

              <Col md={3}>
                <Form.Control type="number" value={tier.max_qty} onChange={(e) => updateTier(i, 'max_qty', e.target.value)} />
              </Col>

              <Col md={3}>
                <Form.Control type="number" value={tier.whole_seller_price} onChange={(e) => updateTier(i, 'whole_seller_price', e.target.value)} />
              </Col>

              <Col md={3}>
                <Button variant="danger" onClick={() => removeTier(i)}>Remove</Button>
              </Col>
            </Row>
          ))}
